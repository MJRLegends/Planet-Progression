			...
			try {
				...
				for (CelestialBody temp : stats.getUnlockedPlanets()) {
					if (body.equalsIgnoreCase(temp.getUnlocalizedName().substring(temp.getUnlocalizedName().indexOf('.') + 1))) {
						...
						playerToAddFor.addChatMessage(new ChatComponentText(temp.getLocalizedName() + " has been removed from your discovered list!"));
						playerBase.addChatMessage(new ChatComponentText(EnumColor.AQUA + "You have remove " + temp.getLocalizedName() + "! from the discovered list for: " + playerToAddFor.getName()));
					}
				}
