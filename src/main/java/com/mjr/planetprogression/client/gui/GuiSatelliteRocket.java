...
public class GuiSatelliteRocket extends GuiContainerGC {
	private static ResourceLocation[] rocketTextures = new ResourceLocation[4];

	static {
		for (int i = 0; i < 4; i++) {
			GuiSatelliteRocket.rocketTextures[i] = new ResourceLocation(Constants.ASSET_PREFIX, "textures/gui/rocket_" + i * 18 + ".png");
		}
	}

	private final IInventory upperChestInventory;
	private final EnumRocketType rocketType;
	private EntitySatelliteRocket rocket;
	private GuiButton launchButton;

	public GuiSatelliteRocket(IInventory par1IInventory, EntitySatelliteRocket rocket) {
		this(par1IInventory, rocket, rocket.getType());
	}
	...
	public void initGui() {
		...
		this.launchButton = new GuiButton(0, var7 + 116, var6 + 26, 50, 20, TranslateUtilities.translate("gui.message.launch.name"));
		this.buttonList.add(this.launchButton);
		List<String> fuelTankDesc = new ArrayList<String>();
		fuelTankDesc.add(TranslateUtilities.translate("gui.fuel_tank.desc.0"));
		fuelTankDesc.add(TranslateUtilities.translate("gui.fuel_tank.desc.1"));
	...
	protected void drawGuiContainerForegroundLayer(int par1, int par2) {
		...
		if (this.rocket.statusMessageCooldown == 0 || this.rocket.statusMessage == null) {
			...
			spltString[0] = TranslateUtilities.translate("gui.cargorocket.status.waiting.0");
			spltString[1] = TranslateUtilities.translate("gui.cargorocket.status.waiting.1");

			if (this.rocket.launchPhase != EnumLaunchPhase.UNIGNITED.ordinal()) {
				spltString = new String[2];
				spltString[0] = TranslateUtilities.translate("gui.cargorocket.status.launched.0");
				spltString[1] = TranslateUtilities.translate("gui.cargorocket.status.launched.1");
	...
	protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
		this.mc.getTextureManager().bindTexture(GuiSatelliteRocket.rocketTextures[(this.rocketType.getInventorySpace() - 2) / 18]);
