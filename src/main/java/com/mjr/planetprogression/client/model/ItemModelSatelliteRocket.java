...
public class ItemModelSatelliteRocket extends ModelTransformWrapper {
	...
	protected Matrix4f getTransformForPerspective(TransformType cameraTransformType) {
		if (cameraTransformType == TransformType.GUI) {
			...
			ret.mul(mul);
			mul.setIdentity();
			mul.setTranslation(new Vector3f(-1.5F, 0.1F, -0.5F));
			ret.mul(mul);
			mul.setIdentity();
		...
		if (cameraTransformType == TransformType.FIRST_PERSON) {
			...
			Vector3f trans = new Vector3f(6.5F, 12.0F, -6.0F);
			Matrix4f ret = new Matrix4f();
			ret.setIdentity();
			Matrix4f mul = new Matrix4f();
			mul.setIdentity();
		...
		if (cameraTransformType == TransformType.THIRD_PERSON) {
			...
			Vector3f trans = new Vector3f(9.4F, -14.4F, 0.1F);
			Matrix4f ret = new Matrix4f();
			ret.setIdentity();
			Matrix4f mul = new Matrix4f();
			mul.setIdentity();
			Quat4f rot = TRSRTransformation.quatFromYXZDegrees(new Vector3f(75, -30, 0));
			mul.setRotation(rot);
			ret.mul(mul);
			mul.setIdentity();
			mul.setScale(0.5F);
		...
		if (cameraTransformType == TransformType.FIXED) {
			...
			ret.mul(mul);
			mul.setIdentity();
			mul.setTranslation(new Vector3f(0.1F, -25F, 0.05F));
			ret.mul(mul);
			return ret;
		}
