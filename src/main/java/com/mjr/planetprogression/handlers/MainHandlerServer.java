...
public class MainHandlerServer {
		...
		if (event.getEntityLiving() instanceof EntityPlayerMP) {
			...
			if (event.getEntityLiving().worldObj.provider instanceof IGalacticraftWorldProvider) {
				...
				if (temp != null && !stats.getUnlockedPlanets().contains(temp)) {
					...
					player.addChatMessage(new TextComponentString(TranslateUtilities.translate("research.discovered.name") + temp.getLocalizedName() + "!"));
					player.addChatMessage(new TextComponentString(TranslateUtilities.translate("research.howto.name")));
				}
			} else {
				if (!stats.getUnlockedPlanets().contains(GalacticraftCore.planetOverworld)) {
					...
					if (Config.showOverworldSpawnMessage) {
						player.addChatMessage(new TextComponentString(TranslateUtilities.translate("research.discovered.earth.name")));
						player.addChatMessage(new TextComponentString(TranslateUtilities.translate("research.howto.name")));
					}
				}
			}
			...
			for (Planet planet : GalaxyRegistry.getRegisteredPlanets().values()) {
				if (list.contains(planet.getUnlocalizedName().toLowerCase().replaceAll("ep", ""))) {
					...
					player.addChatMessage(new TextComponentString(TranslateUtilities.translate("research.discovered.name") + planet.getLocalizedName() + "!"));
					player.addChatMessage(new TextComponentString(TranslateUtilities.translate("research.howto.name")));
				}
			}
			for (Moon moon : GalaxyRegistry.getRegisteredMoons().values()) {
				if (list.contains(moon.getUnlocalizedName().toLowerCase().replaceAll("ep", ""))) {
					...
					player.addChatMessage(new TextComponentString(TranslateUtilities.translate("research.discovered.name") + moon.getLocalizedName() + "!"));
					player.addChatMessage(new TextComponentString(TranslateUtilities.translate("research.howto.name")));
				}
			}
		}
	...
	protected void sendUnlockedPlanetsPacket(EntityPlayerMP player, IStatsCapability stats) {
		...
		PlanetProgression.packetPipeline.sendTo(new PacketSimplePP(EnumSimplePacket.C_UPDATE_UNLOCKED_PLANET_LIST, GCCoreUtil.getDimensionID(player.worldObj), new Object[] { planets }), player);
	}
		...
		if (event.getName().toString().equals("minecraft:chests/simple_dungeon")) {
			if (Config.generateResearchPaperInLoot)
				for (Item item : PlanetProgression_Items.researchPapers)
					if (!Arrays.asList(Config.dungeonPaperBlacklist).contains(((ResearchPaper) item).getBodyName().toLowerCase().replaceAll("ep", "")))
